...
namespace TacticsBoard
{
    ...
    partial class FieldControl : UserControl
    {
        ...
        static public FieldLayout ConvertFieldObjectsToLayout(ICollection<FieldObject> fieldObjects)
        {
            ...
            {
                layout.AddEntry(fo.Tag, fo.Position, fo.rotationangle);
            }
            return layout;
        }
            ...
            if (null != layout)
            {
                foreach (FieldObject fo in fieldObjects)
                {
                    if (layout.HasEntry(fo.Tag))
                    {
                        ...
                        fo.rotationangle = layout.getRotationAngle(fo.Tag);
                        IsDirty = true;
                    }
                }
                Invalidate();
      ...
      throw new NotImplementedException();
        ...
        private void DrawLayoutIntoGraphics(Graphics g, FieldLayout layoutToDraw, FieldLayout nextLayoutData)
        {
            ...
            foreach (FieldObject fieldObject in fieldObjects)
            {
                if (null != layoutToDraw)
                {
                    fieldObject.DrawAt(g, layoutToDraw.GetEntryPosition(fieldObject.Tag), layoutToDraw.getRotationAngle(fieldObject.Tag));
                }
                else
                {
                    fieldObject.DrawAt(g, fieldObject.Position, fieldObject.rotationangle);
                }
            }
        }
        ...
        private void RotateObject(FieldObject fo, PointF pos)
        {
            if (ShowMovementLines && (null != nextLayout))
            {
            ...
            fo.rotationangle += (pos.Y - fo.Position.Y)/5;
            IsDirty = true;
            if (ShowMovementLines && (null != nextLayout))
            {
        ...
        private FieldObject rotateObject;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (null != e)
            {
                ...
                else if((Capture) && (rotateObject != null))
                {
                    PointF pt = ToFieldPoint(new Point(e.X, e.Y));
                    RotateObject(rotateObject, pt);
                }
        ...
        {
            if ((fieldObjectContextMenu.Tag is FieldObject fo) && (null != CustomLabelProvider))
            {
